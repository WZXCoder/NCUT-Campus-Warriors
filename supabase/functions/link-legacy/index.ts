...
function hashPassword(username: string, password: string) {
  // 与前端 store.js 保持一致：base64(utf8(`${username}:${password}`))
...
Deno.serve(async (req) => {
  ...
  // 如果当前 auth 已绑定，直接返回绑定的档案
  const { data: already, error: alreadyErr } = await admin
    .from("users")
    .select("*")
    .eq("auth_user_id", auth.userId)
    .maybeSingle();
  if (alreadyErr) return json(500, { error: "读取用户失败" });
  ...
  const { data: legacy, error: legacyErr } = await admin
    ...
    .maybeSingle();
  if (legacyErr) return json(500, { error: "读取旧账号失败" });
  ...
  if (legacy.password_hash !== expected) return json(403, { error: "旧用户名或密码错误" });

  // 原子绑定：避免并发重复绑定
  const { data: linked, error: linkErr } = await admin
    .from("users")
    .update({ auth_user_id: auth.userId, updated_at: new Date().toISOString() })
    .eq("id", legacy.id)
    .is("auth_user_id", null)
    .select("*")
    .single();
  if (linkErr) return json(500, { error: "绑定失败" });
