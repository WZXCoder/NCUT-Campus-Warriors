-- 用户名+密码 + 数字验证码 + 限流（在 Supabase SQL Editor 执行，可重复执行）
...
immutable
as $$
  select encode(digest(p_id::text || ':' || trim(p_code) || ':ncut_captcha_v1', 'sha256'), 'hex');
$$;

create or replace function public._auth_check_rate_limit(
    p_bucket text,
    p_max integer,
...
as $$
declare
  v_row public.auth_captcha_challenges%rowtype;
  v_hash text;
begin
  if p_id is null or trim(p_code) = '' then
    return false;
  end if;

  select * into v_row from public.auth_captcha_challenges where id = p_id;
  if not found or v_row.used or v_row.expires_at < now() then
    return false;
  end if;

  v_hash := public._auth_captcha_hash(p_id, p_code);
  if v_row.answer_hash is distinct from v_hash then
    return false;
  end if;
...
declare
  ...
  v_code text;
  v_device text := coalesce(trim(p_device_id), '');
begin
  perform public._auth_check_rate_limit(
    'captcha:' || coalesce(nullif(v_device, ''), 'global'),
    30,
    60
  );
...
begin
  ...
  if not public._auth_verify_captcha(p_captcha_id, p_captcha_code) then
  ...
  if exists (select 1 from public.users where username = trim(p_username)) then
